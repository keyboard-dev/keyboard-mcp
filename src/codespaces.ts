...
interface CreateCodespaceParams {
  ...
  export const findCodespaceExecutorRepos = async (token: string): Promise<any | Error> => {
    try {
      ...
      return {
        success: true,
        repositories: allCodespaceRepos,
        count: allCodespaceRepos.length,
        userRepos: codespaceRepos,
        orgRepos: orgRepos
      };
    } catch (e) {
    ...
    try {
      ...
      return {
        ...
        foundRepoInfo: { owner: actualOwner, repo: actualRepo, searchPerformed: !owner || !repo }
      };
    } catch (e) {
      if (e && typeof e === 'object' && 'response' in e) {
        const error = e as { response: { headers: any; status: number }; message: string };
